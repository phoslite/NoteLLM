...
def assign_clusters(db: Session, books: list[Book] | None = None) -> dict[int, str]:
    ...
    keywords = {b.id: extract_keywords(book_corpus(b), 40) for b in pending}
    for b in pending:
        ...
        if tags:
            ...
            continue
        if b.folder_id:
            folder = db.get(Folder, b.folder_id)
            folder_name = sanitize_cluster_name(folder.name) if folder else ""
            if folder_name:
    ...
    for b in books:
        ...
        if tags:
            ...
            folder = db.get(Folder, b.folder_id)
            src = "folder" if folder and sanitize_cluster_name(folder.name) else "pre"
        else:
            src = "pre"
