...
def test_import_triggers_incremental_graph_update(client):
...
def test_knowledge_appears_in_cross_book(client):
    """跨书检索：知识点还出现在哪些书（先构建两本书内图谱，重要段落知识点关键词命中）。"""
    text = "# 第一章 定义与定理\n\n定理 1：完备空间是巴拿赫空间。\n\n# 第二章 证明方法\n\n证明：先证明必要性。\n"
    a = _import_md(client, "跨书A.md", text)
    b = _import_md(client, "跨书B.md", text)
    # 先构建两本书内图谱（懒构建）
    intra_a = client.get(f"/api/graph/books/{a}").json()["data"]
    client.get(f"/api/graph/books/{b}")
...
def test_knowledge_appears_in_rag_key_points(client):
    ...
    client.post(f"/api/graph/books/{a}/rebuild")
    intra = client.get(f"/api/graph/books/{a}").json()["data"]
