...
| 文档 | 版本 | 内容 |
| --- | --- | --- |
| [需求文档.md](需求文档.md) | v1.71 | 需求总纲：概述 / 分册索引 / 变更记录 |
...
| [需求-决策.md](需求-决策.md) | v1.58 | 分册：决策 1~36 / 开放项 |
| [技术栈规范.md](技术栈规范.md) | v1.72 | 技术选型、工程规范、解耦/重构规则 |
| [重构规范.md](重构规范.md) | v1.40 | 重构记录表与变更记录 |
| [docs/使用手册.md](docs/使用手册.md) | v1.109 | 手册总纲：模块地图 / 快速定位 / 跨模块流程 / 变更记录 |
| [docs/使用手册-后端核心.md](docs/使用手册-后端核心.md) | v1.91 | 分册：配置/数据库/解析/导入/任务/API 基础 |
| [docs/使用手册-阅读与PDF.md](docs/使用手册-阅读与PDF.md) | v1.91 | 分册：阅读闭环/书签/涂鸦/页图/视觉提取 |
| [docs/使用手册-AI与资产.md](docs/使用手册-AI与资产.md) | v1.91 | 分册：LLM/对话/脑图/RAG/Skill/Prompt |
| [docs/使用手册-图谱与画像.md](docs/使用手册-图谱与画像.md) | v1.91 | 分册：图谱/聚类/画像/阈值学习 |
| [docs/使用手册-前端.md](docs/使用手册-前端.md) | v1.91 | 分册：前端组件/组合式函数/工具 |
...
- **多模态视觉提取（M7 / 决策 36）**：对 PDF 提问时按 `[P-1, P, P+1]` 滑动窗口调用视觉模型提取页面完整信息，缓存到书籍目录 `page_text/`，缓存命中不重复调用；书架导入**不再自动预提取**（书籍卡片手动「提取全书页缓存」，x/y 页进度、可取消）；资料页知识库投喂时自动批量预提取；读完归档时全书批量提取；**未缓存的附件（划线区域裁剪图 / Markdown 正文插图）统一经视觉模型提取为文本**（内容寻址缓存 `data/cache/attachment_text/`，命中不重复调用），主模型只收文本，消除纯文本端点的 `image_url` 400 报错。
