...
| [技术栈规范.md](技术栈规范.md) | v1.89 | 技术选型、工程规范、解耦/重构规则 |
| [重构规范.md](重构规范.md) | v1.50 | 重构记录表与变更记录 |
| [docs/使用手册.md](docs/使用手册.md) | v1.141 | 手册总纲：模块地图 / 快速定位 / 跨模块流程 / 变更记录 |
| [docs/使用手册-后端核心.md](docs/使用手册-后端核心.md) | 随总纲（当前 v1.141） | 分册：配置/数据库/解析/导入/任务/API 基础 |
| [docs/使用手册-阅读与PDF.md](docs/使用手册-阅读与PDF.md) | 随总纲（当前 v1.141） | 分册：阅读闭环/书签/涂鸦/页图/视觉提取 |
| [docs/使用手册-AI与资产.md](docs/使用手册-AI与资产.md) | 随总纲（当前 v1.141） | 分册：LLM/对话/脑图/RAG/Skill/Prompt |
| [docs/使用手册-图谱与画像.md](docs/使用手册-图谱与画像.md) | 随总纲（当前 v1.141） | 分册：图谱/聚类/画像/阈值学习 |
| [docs/使用手册-前端.md](docs/使用手册-前端.md) | 随总纲（当前 v1.141） | 分册：前端组件/组合式函数/工具 |
...
| [docs/待办清单.md](docs/待办清单.md) | v2.28 | 待办 / 缺口 / 备选方案清单（按紧急度排序，含审查记录） |
...
| 日期 | 内容 |
| --- | --- |
| 2026-08-13 | 四域复审修复轮 3（手册 v1.141）：四域 0 P0；后端 core/services P1×1+P2×4+P3×1（KnowledgePoint.updated_at 并入跨书索引指纹 / reading_logs 唯一约束+存量去重+upsert 转更新 / 增量补边 IntegrityError 回滚重试 / list_tasks rowid 决胜 / 资产共享转移 4 处 bump version / list_books 列裁剪契约 docstring，+7 pytest）；前端 P2×4+P3×5（GraphView backToGlobal 复位 loading + rebuildCurrent 代际/视图守卫 / 高亮缓存内容指纹 + contains 校验 / ReaderView 空书清残留 / taskCenter 防楔死 / hasUnclosedMath 奇偶判定，+6 vitest）；文档 P2×4+P3×11（重构规范 §11 补轮 2 行、core P1×3 补登记、后端核心 §2.4 哨兵/列裁剪契约、需求-决策 §13 指针修正、模块地图补 taskCenter）；验证：后端 339 pytest + ruff、前端 143 vitest + vue-tsc + vite build、markdown-it 17 断言全绿；同步 技术栈 v1.89、重构 v1.50、待办 v2.28 |
| 2026-08-12 | 四域审查修复轮 2（手册 v1.140）：前端 P1×3（滚动 rAF+段落索引 / 笔记高亮增量 / 图谱代际守卫）+ P2×3/P3×4（任务中心关闭竞态 P2-1 / 流式错误文案保护 / 最后消息 watch / 真 LRU / recentBooks 缓存 / 转义 $ 判定 / 历史懒加载）；后端复杂度 H1~H3（聚类合并 12.1x、pair 预计算 4.4x、跨书检索倒排索引 10~12x）+ core P1×3（folder_id 哨兵 / 书架列裁剪 / book_relations 唯一约束）；文档 P1/P2×9+（12 个 md）；验证：后端 332 pytest + ruff、前端 137 vitest + vue-tsc + vite build、markdown-it 断言全绿；同步 技术栈 v1.88、重构 v1.49、待办 v2.27 |
